    ...
    Multicall
{
    ...
    mapping(address => mapping(uint256 => uint256))
        public nextMintTimestampByCondition;
    ...
    function lazyMintBatch(string[] calldata _uris) external {
        ...
        require(
            (nextTokenId + _uris.length) <= maxTotalSupply,
            "exceed maxTotalSupply"
        );
    ...
    function lazyMintAmount(uint256 amount) external {
        require(hasRole(MINTER_ROLE, _msgSender()), "only minter");
        require(
            (nextTokenId + amount) <= maxTotalSupply,
            "exceed maxTotalSupply"
        );
        nextTokenId += amount;
    }

    function claim(uint256 quantity, bytes32[] calldata proofs)
        external
        payable
        nonReentrant
    {
        uint256 conditionIndex = getLastStartedMintConditionIndex();
        PublicMintCondition memory currentMintCondition = mintConditions[
            conditionIndex
        ];

        require(quantity > 0, "need quantity");
        require(
            nextMintTokenId + quantity <= maxTotalSupply,
            "exceed max supply limit"
        );
        require(
            nextMintTokenId + quantity <= nextTokenId,
        ...
        require(
            currentMintCondition.currentMintSupply + quantity <=
                currentMintCondition.maxMintSupply,
            "exceed max mint supply"
        );

        uint256 nextMintTimestampConditionIndex = conditionIndex +
            nextMintTimestampConditionStartIndex;
        uint256 nextMintTimestamp = nextMintTimestampByCondition[_msgSender()][
            nextMintTimestampConditionIndex
        ];
        require(
        ...
        if (currentMintCondition.merkleRoot != bytes32(0)) {
            ...
            require(
                MerkleProof.verify(
                    proofs,
                    currentMintCondition.merkleRoot,
                    leaf
                ),
                "invalid proofs"
            );
        ...
        uint256 newNextMintTimestamp = currentMintCondition
        ...
        nextMintTimestampByCondition[_msgSender()][
            nextMintTimestampConditionIndex
        ] = newNextMintTimestamp;
    ...
    function _transferPaymentWithFee(
        ...
        if (currency == address(0)) {
            require(
                msg.value == saleRecipientAmount + feeAmount,
                "value != amount"
            );
    ...
    function setPublicMintConditions(PublicMintCondition[] calldata conditions)
        external
        onlyModuleAdmin
    {
        ...
        for (uint256 i = 0; i < conditions.length; i++) {
            ...
            require(
                conditions[i].maxMintSupply > 0,
                "max mint supply cannot be 0"
            );
            require(
                conditions[i].quantityLimitPerTransaction > 0,
            ...
            mintConditions.push(
                PublicMintCondition({
                    startTimestamp: block.timestamp +
                        conditions[i].startTimestamp,
                    maxMintSupply: conditions[i].maxMintSupply,
                    currentMintSupply: 0,
                    quantityLimitPerTransaction: conditions[i]
                        .quantityLimitPerTransaction,
                    waitTimeSecondsLimitPerTransaction: conditions[i]
                        .waitTimeSecondsLimitPerTransaction,
    ...
    function setRestrictedTransfer(bool _restrictedTransfer)
        external
        onlyModuleAdmin
    {
